...
def get_risk_detail(risk_id: int):
    ...
    if risk.get("entity_type") == "EMPLOYEE" and risk.get("entity_id"):
        ...
        vehicle = db1(
            ...
            "WHERE employee_id=? AND active=1 LIMIT 1",
            (risk["entity_id"],)
        )
...
@app.get("/api/risks/{risk_id}/photo2")
def get_risk_photo2(risk_id: int):
    row = db1("SELECT photo_path_2 FROM risks WHERE id=?", (risk_id,))
    file_path = row.get("photo_path_2")
    if not file_path or not Path(file_path).exists():
        return Response(status_code=404)
    return FileResponse(file_path)
